...
const initActionTrendChart = async () => {
  ...
  try {
    ...
    // 获取真实的用户行为数据
    // 注意：这里为了简化逻辑直接拉取了10000条记录进行前端统计
    // 在生产环境数据量大时，应该由后端提供按日期聚合的接口
...
const initUserActivityChart = async () => {
  ...
  try {
    ...
    const option = {
      ...
      color: generateColors(activityData.length),
      series: [
        {
          name: '用户数量',
          type: 'bar',
...
const initActionTypeChart = async () => {
  ...
  try {
    ...
    try {
      ...
      favoriteCount = Math.floor(viewCount * 0.1) // 假设收藏率为10%作为兜底
    }
    ...
    const option = {
      ...
      color: generateColors(actionData.length),
      series: [
        {
          name: '行为数量',
          type: 'pie',
  ...
  try {
    ...
    for (const [itemId, stats] of itemStats.entries()) {
      try {
        ...
        // 如果获取失败，使用浏览数据的估算值
        stats.favorite = Math.floor(stats.view * 0.15)
      }
    }
